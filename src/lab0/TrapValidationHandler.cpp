...
bool TrapValidationHandler::isAllowed(const Figure& fugure) const {
    try {
        dynamic_cast<const Trap &>(fugure);
...
void TrapValidationHandler::validate(const Figure& figure) const {
    for (int i = 0; i < 4; i++) {
    ...
    if (!isParalel(figure.getPoints()[0] - figure.getPoints()[1], figure.getPoints()[2] - figure.getPoints()[3])){
